...
exports.default = function (_ref2) {
  ...
  let expect = _ref2.expect;

  const app = (0, _express2.default)();
  const everything$ = new _rx2.default.Subject();
  if (expect) {
    if (!parsers[expect]) {
      throw new Error(`Bad 'expect' configuration. No builtin parser for '${ expect }'.`);
    } else {
      app.use(parsers[expect]());
    }
  }
  if (middlewares) {
    app.use.apply(app, _toConsumableArray(middlewares));
  }
...
const parsers = {
  json: _bodyParser.json,
  text: _bodyParser.text,
  urlencoded: _bodyParser.urlencoded.bind(null, { extended: true })
};
